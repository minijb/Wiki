...
> 基于 LearnOpenGL 教程的实践笔记，涵盖窗口创建、渲染管线、着色器编程。
...
OpenGL 是一个**状态机**。所有状态与 `OpenGL Context` 关联。操作 OpenGL 就是操作不同的状态（如绑定缓冲区、操作缓冲区、使用当前缓冲区渲染）。
...
glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);  // 3. 填充数据
```
...
每个顶点属性从**当前绑定到 `GL_ARRAY_BUFFER` 的 VBO** 中获取数据。
...
VAO 中存储 attribute pointer 数组，因此可以随时解绑 VBO。绑定的 EBO 直接存储在 VAO 中，解绑 VAO 后可以安全解绑 EBO。
...
**分量访问方式：** `.xyzw`（位置）或 `.rgba`（颜色）或 `.stpq`（纹理）。
...
NDC（Normalized Device Coordinates）：屏幕坐标中心为 (0,0)，范围 (-1,-1) 到 (1,1)。

### 5.3 GPU 管线概览（GAMES202）
...
| 工具 | 说明 |
|:-----|:-----|
| **Nsight Graphics** | NVIDIA 图形调试器 |
| **RenderDoc** | 跨平台图形调试器，支持截帧分析 |
...
- **黑屏无输出**：检查着色器编译/链接状态、VAO 绑定状态
- **着色器编译失败**：使用 `glGetShaderInfoLog` 获取错误信息
- **链接失败**：检查顶点着色器输出与片元着色器输入的变量名是否匹配
- **VAO/EBO 顺序错误**：EBO 必须在 VAO 绑定时绑定
