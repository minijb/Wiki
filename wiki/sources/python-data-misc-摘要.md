...
Python 性能检测、图像处理（PIL / OpenCV / Matplotlib 三库格式互转）、MySQL 基础操作与远程连接、JavaScript 基础语法等杂项知识汇总。Python 文件 I/O 和子进程管理已有独立专题页面。

## 要点

1. **性能检测四件套**：`cProfile`（函数级统计）→ `snakeviz`（火焰图可视化）→ `line-profiler`（逐行耗时）→ `timeit`（微基准测试），逐级细化定位瓶颈
